...
from database_mobile import SessionLocal, engine, Base
import models_mobile
from models_mobile import Utilisateur, Restaurant, Reservation, Message
from schemas_mobile import UtilisateurCreate, UtilisateurResponse, ConnexionSchema, ConnexionResponse, ReservationCreate, MessageCreate, AvisCreate
...
def inserer_restaurants_en_bdd(db: Session):
    for r in restaurants_data:
        existing = db.query(Restaurant).filter_by(adresse=r.get("adresse")).first()
        if not existing:
...
def inscription(utilisateur: UtilisateurCreate, db: Session = Depends(get_db)):
    ...
    nouveau_utilisateur = Utilisateur(
        ...
        mdp=utilisateur.mdp  # ⚠️ À sécuriser avec du hashing en prod
    )
...
def get_avis_by_restaurant(restaurant_id: int, db: Session = Depends(get_db)):
    avis_list = db.query(models_mobile.Avis).filter(models_mobile.Avis.restaurant_id == restaurant_id).all()
    result = []
    for avis in avis_list:
        utilisateur = db.query(Utilisateur).filter(Utilisateur.id == avis.utilisateur_id).first()
        result.append({
            "id": avis.id,
            "utilisateur_id": avis.utilisateur_id,
